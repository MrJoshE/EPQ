class Matrix{
  ...
  static fromArrayToMatrix(array){
    //Converts arrays to matricies. So that the inputs can be translated
    //into a matrix to perform the elsewid
  ...
  //Having the function as static means that it can be called in the form
  ...
  static transpose(m1){
    ...
    //m1 is the matrix that is entered into the function.
    //this function turns a matrix with 3 rows and 2 collums into a
    //matrix with 2 rows and 3 collumns


    let results = new Matrix(m1.cols,m1.rows)


    for(let i = 0; i < m1.rows; i++){
      for(let j = 0; j < m1.cols; j++){
        results.matrix[j][i] = m1.matrix[i][j];
      }
    }
    return results;
  ...
  map(func){
    //Applies a function such as add to every element in the data.

    for(let i = 0; i < this.rows; i++){
      for (let j = 0; j < this.cols; j++){
        //Passes each value in the matrix into the function
